...
class TableclothDisplay(object):
    ...
    def __init__(self, window, bkg_image, folder):
        ...
        self.bkg0 = pyglet.sprite.Sprite(image, subpixel=1,
                                         batch=self.background)
        self.bkg1 = pyglet.sprite.Sprite(image, subpixel=1,
                                         batch=self.background)
...
def new_photo(dt=0):
    filename = 'photo.jpg'
    files = glob.glob('/var/lib/iii/0018562a8795/*.JPG')
    if not files: return
...
def check_queue(dt):
    ...
    try:
        ...
        sprite = pyglet.sprite.Sprite(image, x=x, y=y,
                subpixel=True, batch=image_batch)
        images.append(sprite)

#pyglet.clock.set_fps_limit(60)
pyglet.clock.schedule(scroll)
pyglet.clock.schedule_interval(new_photo, 6)
